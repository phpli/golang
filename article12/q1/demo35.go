...
func main()  {
		...
		//_ = &([3]int{1,2,3,4}[0])对数组字面量的索引结果值不可寻址。
		//_ = &([3]int{1,2,3,4}[0:3])// 对数组字面量的切片结果值不可寻址。
		_ = &([]int{1,2,3,4}[0]) // 对切片字面量的索引结果值却是可寻址的。
		//_ = &([]int{1,2,3,4,5}[0:2])// 对切片字面量的切片结果值不可寻址。
		//_ = &(map[int]string{1:"a"}[0])// 对字典字面量的索引结果值不可寻址。
		var map1 = map[int]string{1:"a",2:"b",3:"c"}
		_ = map1
		//_ = &map1[1] // 对字典变量的索引结果值不可寻址
		//_ = &func(x,y int)int {
		//	return  x+y
		//}// 字面量代表的函数不可寻址。
		//_ = &(fmt.Printf("a"))// 对函数的调用结果值不可寻址。
